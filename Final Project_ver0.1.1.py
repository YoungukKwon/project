...
file_list = [["platform1.gif", 100], ["platform2.gif", 100], ["platform3.gif", 100], ["platform4.gif", 100]]
score = 0
class Figure(Sprite):
    def __init__(self, game):
        Sprite.__init__(self, game)
    ...
    def move(self):     # Main movement in every loop
        ...
        if co.y2 >= self.game.canvas_height:  # When rabbit reaches the ground, game stops and 'game over pop up'
            self.y = 0
            self.x = 0
            self.gameover = self.game.canvas.create_text(250, 350, text="Game Over", font=("Helvetica", 20, "bold"), fill="red") # changjin
            self.game.running = False
            self.game.canvas.bind_all('<KeyPress-Return>', self.restart) # changjin: restart game
        self.game.canvas.move(self.image, self.x, self.y)
        self.game.canvas.move(self.message, self.x, self.y) #changjin
        ...
        if self.game.canvas.coords(self.image)[1] <= 300:   ## when rabbit reaches the half of screen, all objects go down
            ...
            if time.time()-self.last_time > 0.2:    # new platform occurs from above when screen moves in constant interval
                platform_new = Platform(g, PhotoImage(file=file_list[r][0]), random.randint(0,340), self.game.next_y, file_list[r][1], 10)
                self.game.sprites.append(platform_new)
                self.game.next_y -= random.randint(40,60) #changjin

    def restart(self, event): # changjin
        self.game.canvas.delete(self.gameover) #<<<<<<----------------- 이부분이 게임오버 메시지를 지우는 부분인데 제 기능을 못하고 있습니다.(오류없이 실행이되긴함.)
...
def initial_setting(g): #changjin
    ...
    platform3 = Platform(g, PhotoImage(file="platform3.gif"), 100, 430, 100, 10)
    platform4 = Platform(g, PhotoImage(file="platform4.gif"), 160, 300, 100, 10)
    platform5 = Platform(g, PhotoImage(file="platform4.gif"), 300, 240, 60, 10)
    platform6 = Platform(g, PhotoImage(file="platform3.gif"), 70, 170, 100, 10)
