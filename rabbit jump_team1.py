...
### Team1 Final Project
### Version 3
### version file upload: https://github.com/YoungukKwon/project
...
class Game:
    def __init__(self):
        ...
        self.sprites = []  # sprites: list containing platform and rabbit
        ...
        self.gameover = self.canvas.create_text(250, 350, text="Game Over", font=("Helvetica", 20, "bold"), fill="red",
                                                state=HIDDEN)
        self.restart_message = self.canvas.create_text(250, 380, text="Press Enter to restart", font=("Helvetica", 10, "bold"), state=HIDDEN)
...
class Figure(Sprite):
    ...
    def move(self):  # Main playing method in every loop
        ...
        if self.game.canvas.coords(self.image)[1] <= 300:  # when rabbit reaches the half of screen, screen goes down for 20 pix and all objects goes up 20 pix.
            ...
            self.game.canvas.move(self.game.restart_message, 0, -20)
    ...
    def chance(self):     # method for restart game after game over
        ...
        self.game.canvas.itemconfig(self.game.restart_message, state=NORMAL)
    ...
    def restart(self, event):     # restart game after gameover
        if self.game.running == False and self.game.canvas.coords(self.image)[1] + 47 >= self.game.canvas_height:
            ...
            self.game.canvas.itemconfig(self.game.restart_message, state=HIDDEN)
