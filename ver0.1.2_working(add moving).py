...
class MovingPlatform(Platform):
    ...
    def move(self):
        if time.time() - self.last_time > 0.03:
            ...
            # self.change_count += 1
            co = self.coords()
            if co.x2 >= 450 or co.x1 <= 0:
                self.x = self.x * (-1)

## Figure(Rabbit, Design) class
file_list = [["platform1.gif", 100, "A"], ["platform2.gif", 100, "B"], ["platform3.gif", 100, "C"], ["platform4.gif", 100, "D"]]
score = 0
class Figure(Sprite):
    def __init__(self, game):
        Sprite.__init__(self, game)
    ...
    def move(self):     # Main movement in every loop
        ...
        if co.y2 >= self.game.canvas_height:  # When rabbit reaches the ground, game stops and 'game over pop up'
            self.y = 0
            self.x = 0
            # gameover = self.game.canvas.create_text(250, 350, text="Game Over", font=("Helvetica", 20, "bold"), fill="red") # changjin
        ...
        if self.game.canvas.coords(self.image)[1] <= 300:   ## when rabbit reaches the half of screen, all objects go down
            ...
            if time.time()-self.last_time > 0.2:    # new platform occurs from above when screen moves in constant interval
                platform_type = file_list[r]
                if platform_type[2] != "C":
                    platform_new = Platform(g, PhotoImage(file=platform_type[0]), random.randint(0,340), self.game.next_y, platform_type[1], 10)
                    self.game.sprites.append(platform_new)
                else:
                    platform_new = MovingPlatform(g, PhotoImage(file=platform_type[0]), random.randint(0,340), self.game.next_y, platform_type[1], 10)
                    self.game.sprites.append(platform_new)
...
def initial_setting(g): #changjin
    ...
    platform2 = Platform(g, PhotoImage(file="platform1.gif"), 300,500, 100, 10)
    ...
    platform5 = Platform(g, PhotoImage(file="platform1.gif"), 300, 240, 100, 10)
    platform6 = Platform(g, PhotoImage(file="platform1.gif"), 70, 170, 100, 10)
    platform7 = Platform(g, PhotoImage(file="platform1.gif"), 250, 100, 100, 10)
